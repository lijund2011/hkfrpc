...
clear
echo && echo -e " 
 ${Green_font_prefix}${FRP_TITLE}${Font_color_suffix} ${Red_font_prefix} 版本:v${FRP_VERSION}${Font_color_suffix}
...
${Red_font_prefix}————安装配置客服端————${Font_color_suffix}
 ...
 ${Green_font_prefix}2.${Font_color_suffix} 安装 配置文件 
 ${Green_font_prefix}3.${Font_color_suffix} 编辑配置文件
${Red_font_prefix}——————FRPC客服端——————${Font_color_suffix}
 ${Green_font_prefix}4.${Font_color_suffix} 启动FRPC客服端
 ${Green_font_prefix}5.${Font_color_suffix} 重启FRPC客服端
 ${Green_font_prefix}6.${Font_color_suffix} 停止FRPC客服端
———————————————————————
...
check_status(){
	if [ -f "/usr/local/frp/${FRP_NAME}" ] || [ -f "/lib/systemd/system/${FRP_NAME}.service" ];then
...
echo -e "${Info} ${Red_font_prefix}正在下载 FRPC客服端${Font_color_suffix}"
	...
	wget -N --no-check-certificate -d --show-progress ${WORK_PATH} https://raw.githubusercontent.com/lijund2011/hkfrpc/main/hkfrp.sh && chmod +x hkfrp.sh && bash hkfrp.sh
else
    if [ $PROXY_HTTP_CODE == "200" ]; then
	echo -e "${Info} ${Red_font_prefix}正在使用代理下载 FRPC客服端${Font_color_suffix}"
		...
		wget -N --no-check-certificate -q --show-progress ${WORK_PATH} ${PROXY_URL}https://raw.githubusercontent.com/lijund2011/hkfrpc/main/hkfrp.sh && chmod +x hkfrp.sh && bash hkfrp.sh
    else
        echo -e "${Red}检测 GitHub Proxy 代理失效 开始使用官方地址下载${Font}"
        wget -q --show-progress ${WORK_PATH} https://github.com/fatedier/frp/releases/download/v${VER}/${FILE_NAMEG}.tar.gz -O ${FILE_NAMEG}.tar.gz
		wget -N --no-check-certificate -q --show-progress ${WORK_PATH} https://raw.githubusercontent.com/lijund2011/hkfrpc/main/hkfrp.sh && chmod +x hkfrp.sh && bash hkfrp.sh
    fi
fi
# 显示解压过程 xzvf 不显示解压过程 xzf
